module.exports = function(async, Dept,Users){
    return{
        ...
        postResults: function(req, res){
           async.parallel([
               function(callback){
                   const regex = new RegExp((req.body.region), 'gi');
                   
                   Dept.find({'$or': [{'region': regex}, {'name': regex}]}, (err, result) => {
                       callback(err, result);
                   });
               }
           ], (err, results)  => {
               ...
               res.render('results', {title:'HPCL- Results',  user: req.user, chunks: dataChunk});
           }) 
        },
        ...
        viewMembers: function(req, res){
            ...
            async.parallel([
               ...
               res.render('members', {title:'HPCL- Members',  user: req.user, chunks: dataChunk});
           }) 
        },
